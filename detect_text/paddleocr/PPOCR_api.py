...
class PPOCR_pipe:
    ...
    def runDict(self, writeDict: dict):
        ...
        # 输入信息
        writeStr = jsonDumps(writeDict, ensure_ascii=False, indent=None)+"\n"
        try:
    ...
    def printResult(res: dict):
        ...
        if res["code"] == 100:
            ...
            for line in res["data"]:
                print(f"{index}-location：{line['box']}，文本：{line['text']}")  #origin:print(f"{index}-置信度：{roundline['score']}，文本：{line['text']}")
                index+=1
        elif res["code"] == 100:
            print("图片中未识别出文字。")
        else:
